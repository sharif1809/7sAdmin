...
import { getUpdateCmpCategoryList, getCountryList, uploadImage, getCampaignInfo, getCPCInfo, updateNativeCampaign } from '../../app/api';
...
export default function EditNativeCampaign ({clkFunc}) {
    ...
    const submitAd = async (data) => {
      ...
      if(txt.device_type.length > 0 && txt.device_os.length > 0) {
        ...
        if((data.pricing_model === 'CPC' && parseFloat(data.cpc_amt) >= parseFloat(cpcost)) || data.pricing_model === 'CPM') {
          ...
          const res = await updateNativeCampaign(data);
