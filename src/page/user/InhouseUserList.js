...
const InhouseUsersList = () => {
  ...
  const getUsersData = async (cat,type = "", acnt_type = 1, src = "", usertype = 2) => {
    setLoading(true);
    const res = await getInhouseUsersList(cat,type, acnt_type, src, usertype);
    if (res.data) {
      setData(res.data);
    }
    setLoading(false);
    ...
    <React.Fragment>
      ...
      <Content>
        ...
        <BlockHead size="sm">
          <BlockBetween>
            <BlockHeadContent>
              <BlockTitle>Inhouse User's List</BlockTitle>
              <BlockDes>You have total {data.length} Inhouse user's</BlockDes>
            </BlockHeadContent>
            <BlockHeadContent>
                          ...
                          <DataTableItem key={key}>
                            <DataTableRow>
                              <div className="user-info">
                                <Link
                                  to={`${process.env.PUBLIC_URL}/user-details/${btoa(item.uid)}`}
                                  title="User detail"
                                >
