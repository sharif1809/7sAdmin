...
const InboxReplyItem = ({ reply, replyTo, forwardTo, deleteMessage }) => {
  ...
  const downloadAttachments = (at) => {
    ...
    //  link.href = 'http://192.168.18.166/7sapp/7searchBackend/public/images/support/'+at;
      link.href = 'https://services.7searchppc.com/public/images/support/'+at;
      link.click();
    // });
  };
        ...
        <React.Fragment>
          <div className={`nk-reply-header nk-ibx-reply-header`}>
            <div className="nk-reply-desc">
              <UserAvatar className={`nk-reply-avatar ${reply.created_by == 'User' ? '' : 'bg-success'}`} text={findUpper(reply.user_name)} theme={reply.theme} image={reply.img} />
              <div className="nk-reply-info">
                ...
                </div>{reply.created_by == 'User' ? 'Customer' : 'Support Team'}
