...
const InboxMessages = ({
  ...
  const updateSupportStatus = async (id,sts) => {
    setLoading(true);
  ...
  const sendInbox = async () => {
    ...
    if(attachmentList != null) {
            ...
            form.append('support_type', 'Admin')
            form.append('msgstatus', statusText)
    ...
    <Loader visible={loading} />  
        <React.Fragment>
          ...
          <SimpleBar className="nk-ibx-reply nk-reply">
            ...
            <div className="custom-header-msg">
            <ul className="nk-msg-actions">
              <li>
              {
                  (support) ?
                  (support.status == 1) ?
                   <span className="feedback-msg-color">
                      {support && support.category.toUpperCase()} is Open
                   </span>
                  : (support.status == 2) ? 
                     <span className="feedback-msg-color">
                     {support && support.category.toUpperCase()} is In Progress
                    </span>
                  : (support.status == 3) ? 
                   <span className="feedback-msg-color">
                    {support && support.category.toUpperCase()} is  On Hold
                  </span>
                  : (support.status == 4) ? 
                   <span className="feedback-msg-color">
                    {support && support.category.toUpperCase()} is Customer Action Pending
                 </span>
                  : (support.status == 6) ? 
                  <span className="feedback-msg-color">
                    {support && support.category.toUpperCase()} is ReOpen
                  </span>
                  :
                  <span className="feedback-msg-color">
                    {support && support.category.toUpperCase()} is Closed
                  </span>
                  : ''
                }
                </li>
            ...
            <ul className="nk-msg-actions">
              <li>
                <div className="custom-control custom-control-sm custom-radio custom-control-pro">
                  <input type="radio"   value={1} className="custom-control-input" name="msgstatus" id="msgstatus1"  onChange={(e) => setStatusText(e.target.value)}  />
                  <label className="custom-control-label" htmlFor="msgstatus1">
                    <span> Open </span>
                  </label>
                  </div>
                </li>
                <li>
                  <div className="custom-control custom-control-sm custom-radio custom-control-pro">
                    <input type="radio" className="custom-control-input"  value={5}  name="msgstatus" id="msgstatus2"  onChange={(e) => setStatusText(e.target.value)}/>
                    <label className="custom-control-label" htmlFor="msgstatus2">
                ...
                <li>
                  <div className="custom-control custom-control-sm custom-radio custom-control-pro">
                    <input type="radio" className="custom-control-input"  value={2}  name="msgstatus" id="msgstatus3"  onChange={(e) => setStatusText(e.target.value)} />
                    <label className="custom-control-label" htmlFor="msgstatus3">
                      <span>Mark as progress</span>
                    </label>
                  </div>
                </li>
            </ul>
